  ...
  return (
    <div className="overflow-auto h-[75%] md:h-[75%] bg-white rounded-t-2xl py-4 flex flex-col p-5">
      {mensaje.map((respuesta, index) => (
        <div key={index} className={`w-full flex ${respuesta.emisor === "chat" ? "justify-start" : "justify-end"} mb-4`}>
          {respuesta.emisor === "chat" ? (
            <div ref={mesajeReferencia} className={estilo}>
              {respuesta.texto}
            </div>
          ) : (
            <div ref={mesajeReferencia} className=" rounded-s-xl rounded-tr-xl bg-[#0c2342] max-w-[80%] p-2 py-2 text-gray-200">
              {respuesta.texto}
            </div>
          )}
        </div>
